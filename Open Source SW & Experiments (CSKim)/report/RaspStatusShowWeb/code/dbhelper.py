...
class DBHelper:
    def __init__(self, host='192.168.1.210'):
    ...
    def buildStatusDFFromDB(self, num=None):
        ...
        for row in cursor:
            df_data['time'].append(row[1].strftime("%M:%S.%f")[:-5])
            stat = row[2].split(",")

            df_data['Temp'].append(float(stat[0]))
            df_data['CPU'].append(float(stat[1]))

            memPercent = (float(stat[3]) * 100) / float(stat[2])
            df_data['Mem'].append(float(memPercent))
