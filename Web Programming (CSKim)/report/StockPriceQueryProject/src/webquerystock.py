...
def home():
    if request.method == 'POST':
        ...
        print('profile=', queryObj)
        session['profile'] = queryObj
        return redirect(url_for('home'))

    profile_param = None
    if session.get('profile'):
        profile_param = session.get('profile')
        print('profile_param=', profile_param)

    return render_template("home.html", profile_param=profile_param)
...
def listprice():
    ...
    if request.method == 'POST':
        ...
        queryObj = {'jm_code': jm_code, 'jm_name': jm_name, 'start_day': start_day, 'end_day': end_day,
                    'limit': query_limit}
